...
Vex.Flow.TabDiv.prototype.init = function(sel) {
  ...
  this.notation = $(sel).attr("notation") || true;
  this.parser = $(sel).attr("parser") || "VexTab"; // allow parser to be specified
  ...
  switch (this.parser)
  {
	  ...
	  {
		  this.notation = false;
		  this.parser = new Vex.Flow.Parser.MusicXML(this.notation);
		  break;
	  }
	  case "VexTab":
	  {
...
Vex.Flow.TabDiv.prototype.drawInternal = function() {
  ...
  for (var i = 0; i < staves.length; ++i) {
    ...
    {
	  if (voice_ties[j] === undefined) continue;
